import java.io.*;
import java.util.*;

public class Main {
   static class Edge {
      int src;
      int nbr;

      Edge(int src, int nbr) {
         this.src = src;
         this.nbr = nbr;
      }
   }
   
   // code starts from here
   public static class DFSPair{
       int vertices;
       String psf;
       DFSPair(int vertices , String psf){
           this.vertices = vertices;
           this.psf = psf;
       }
   }
   public static void DFSIterative(ArrayList<Edge>[] graph , int src){
       Stack<DFSPair> st = new Stack<>();
       st.push(new DFSPair(src , src+""));
       boolean visited[] = new boolean [graph.length];
       
       while(st.size() > 0){
           DFSPair tPair = st.pop();
           
           if(visited[tPair.vertices] == false){
               visited[tPair.vertices] = true;
               
               System.out.println(tPair.vertices+"@"+tPair.psf);
               
               for(Edge e : graph[tPair.vertices]){
                   if(visited[e.nbr] == false){
                       st.push(new DFSPair(e.nbr , tPair.psf+e.nbr));
                   }
               }
           }
       }
   }

   public static void main(String[] args) throws Exception {
      BufferedReader br = new BufferedReader(new InputStreamReader(System.in));

      int vtces = Integer.parseInt(br.readLine());
      ArrayList<Edge>[] graph = new ArrayList[vtces];
      for (int i = 0; i < vtces; i++) {
         graph[i] = new ArrayList<>();
      }

      int edges = Integer.parseInt(br.readLine());
      for (int i = 0; i < edges; i++) {
         String[] parts = br.readLine().split(" ");
         int v1 = Integer.parseInt(parts[0]);
         int v2 = Integer.parseInt(parts[1]);
         graph[v1].add(new Edge(v1, v2));
         graph[v2].add(new Edge(v2, v1));
      }

      int src = Integer.parseInt(br.readLine());

      // write your code here  
      DFSIterative(graph , src);
   }
}